'use server';
import { z } from 'zod';
import { sql, Query, QueryParse, QueryResult, QueryResultRow, QueryConfig } from '@vercel/postgres';
...
export async function getPostData(): Promise<PostData[]> {
    const queryConfig: QueryConfig = {
        text: `SELECT id, content, time, file_name_one, file_name_two, file_name_three FROM posts WHERE hidden = '0' ORDER BY time DESC LIMIT 30`
    };
...
export async function getPostDataSearch(searchText: string): Promise<PostData[]> {
